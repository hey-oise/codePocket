...
function Links({ href, text, isActive = false }) {
  if (isActive) {
    return (
      <Link
        className="capitalize bg-gray-700 text-pink-100 w-full p-1 rounded-md pl-2"
        href={href}
      >
        {text}
      </Link>
    ...
    return (
      <Link
        className="capitalize bg-gray-900 transition-100 hover:bg-gray-800 text-pink-100 w-full p-1 rounded-md pl-2"
        href={href}
      >
        {text}
      </Link>
  ...
  return (
    <div className="flex flex-col justify-between gap-10 py-8 bg-gray-950 w-50 p-2 rounded-lg fixed left-1 top-1 bottom-1 text-white max-h-screen">
      <div className="flex flex-col gap-10">
        <p className="capitalize font-bold text-2xl text-gray-400">
          codePocket
        </p>
